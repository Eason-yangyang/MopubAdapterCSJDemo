...
- (void)requestAdWithAdapterInfo:(NSDictionary *)info adMarkup:(NSString *)adMarkup {
    ...
    self.appId = [info objectForKey:kCSJAppIdKey];
    ...
    self.adPlacementId = [info objectForKey:kCSJPlacementIdKey];
    ...
    NSString *userId = [CSJAdapterConfiguration userId];
    if (userId && [userId isKindOfClass:[NSString class]] && userId.length > 0) {
        model.userId = userId;
    }
    
    NSString *rewardName = [CSJAdapterConfiguration rewardName];
    if (rewardName && [rewardName isKindOfClass:[NSString class]] && rewardName.length > 0) {
        model.rewardName = rewardName;
    }
    
    if ([CSJAdapterConfiguration rewardAmount] != 0) {
        model.rewardAmount = [CSJAdapterConfiguration rewardAmount];
    }
    
    NSString *extra = [CSJAdapterConfiguration mediaExtra];
    ...
    if (hasAdMarkup) {
        MPLogInfo(@"Loading CSJ rewarded video ad markup for Advanced Bidding");

        [RewardedVideoAd setMopubAdMarkUp:adMarkup];
    } else {
        MPLogInfo(@"Loading CSJ rewarded video ad");
...
- (void)presentAdFromViewController:(UIViewController *)viewController {
    if ([self hasAdAvailable]) {
        ...
        NSError *error = [NSError
                          ...
                          userInfo:@{NSLocalizedDescriptionKey : @"Failed to show CSJ rewarded video."}];
