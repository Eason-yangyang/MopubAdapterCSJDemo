...
- (void)requestAdWithAdapterInfo:(NSDictionary *)info adMarkup:(NSString *)adMarkup {
    ...
    if (info.count == 0) {
        NSError *error = [NSError errorWithDomain:NSStringFromClass([self class])
                                         ...
                                         userInfo:@{NSLocalizedDescriptionKey:
                                                        @"Incorrect or missing CSJ App ID or Placement ID on the network UI. Ensure the setting is correct on the MoPub dashboard."}];
    ...
    if (hasAdMarkup) {
        MPLogInfo(@"Loading CSJ interstitial ad markup for Advanced Bidding");

        [self.fullScreenVideo setMopubAdMarkUp:adMarkup];
    } else {
        MPLogInfo(@"Loading CSJ interstitial ad");
...
- (void)presentAdFromViewController:(UIViewController *)viewController {
    if (!self.fullScreenVideo || !self.adHasValid) {
        NSError *error = [NSError errorWithDomain:NSStringFromClass([self class])
                                         ...
                                         userInfo:@{NSLocalizedDescriptionKey: @"Failed to show CSJ intersitial ad."}];
