...
@interface CSJNativeAdRenderer () <MPNativeAdRendererImageHandlerDelegate>

@property (nonatomic, strong) UIView<MPNativeAdRendering> *adView;
@property (nonatomic, strong) CSJNativeAdAdapter *adapter;
...
@implementation CSJNativeAdRenderer
...
+ (MPNativeAdRendererConfiguration *)rendererConfigurationWithRendererSettings:(id<MPNativeAdRendererSettings>)rendererSettings {
    ...
    config.supportedCustomEvents = @[@"CSJNativeCustomEvent"];
    
    return config;
}

- (UIView *)retrieveViewWithAdapter:(id<MPNativeAdAdapter>)adapter error:(NSError **)error {
    if (!adapter || ![adapter isKindOfClass:[CSJNativeAdAdapter class]]) {
        if (error) {
    ...
    self.adapter = (CSJNativeAdAdapter *)adapter;
